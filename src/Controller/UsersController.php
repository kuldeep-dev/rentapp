           ...
           // exit;
            ...
            if ($new_user) {
                ...
                if (isset($user)) { 
                 ...
                 $send = $email->from(['kuldeepjha@avainfotech.com' => 'Rental App']) 
                        ->emailFormat('html')
                        ->template('invite')
                        ->to($user->email)
                        ->subject('Welcome to Rental App')
                        ->viewVars(array('link' => $refer_link)) 
                        ->viewVars(array('user' => $user))    
                        ->send();   

                        
                        $this->Flash->success(__('Registration done successfully. We have sent a verification mail to your registered email address, Please verify your account.'));   
            ...
            if ($this->request->data['name'] == '' || $this->request->data['email'] == '' || $this->request->data['password1'] == '' || $this->request->data['password'] == '') {
                ...
                if (!empty($user_check)) {
                    ...
                    if ($new_user) {
                        ...
                        //  $email = new Email('default');
                        //  $email->from(['kuldeepjha@avainfotech.com' => 'Rental App'])  
                        //  	->emailFormat('html')
    ...
    public function forgot() {
        ...
        if ($this->request->is('post')) {
            ...
            if (empty($user)) {
              ...
              $response['data'] = "no data";
              $response['isSucess'] = "false";
              $response['msg'] = "<div class='alert alert-danger'><strong>Enter regsitered email address to reset you password</strong></div>";
                ...
                if ($user->email) {
                 ...
                 $send = $email->from(['kuldeepjha@avainfotech.com' => 'Rent app']) 
                        ->emailFormat('html')
                        ->template('forgot')
                        ->to($user->email)
                        ...
                        $response['data'] = "no data";
                        $response['isSucess'] = "true";
                        $response['msg'] = "<div class='alert alert-success'><strong>Check your email to reset your Password</strong></div>";
                   // $this->Flash->success(__('Check your email to reset your Password'));

                } else {
                      $response['data'] = "no data";
                      $response['isSucess'] = "false";
                      $response['msg'] = "<div class='alert alert-danger'><strong>Email Is Invalid</strong></div>";
    ...
    public function contact() {  
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Contacts->save($contact)) {
                ...
                $email->from(['kuldeepjha@avainfotech.com' => 'Rent app'])
                        ->emailFormat('html')
                        ->template('default', 'default')
    ...
    public function gplogin() {
        ...
        if (isset($this->request->data['action']) && $this->request->data['action'] == "gplogin") {
            ...
            if (!empty($results)) {
                ...
                $post['name'] = $this->request->data['first_name'];
